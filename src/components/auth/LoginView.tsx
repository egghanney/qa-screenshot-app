    ...
    try {
      if (isSetupMode) {
        ...
        if (!setupRes.ok || setupData.error) {
          setError(setupData.error || 'Failed to initialize administrator account.');
          setLoading(false);
          return;
        }
        ...
        if (!res.success) {
          setError(res.error || 'Admin created, but sign-in failed. Please try signing in again.');
        }
      } else {
        // Standard Sign In
        const res = await signIn(email, password);
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative flex items-center">
              ...
              <input
                ...
                placeholder={isSetupMode ? 'Create admin password' : '••••••••••••'}
                minLength={6}
          ...
          >
            {loading ? (
              ...
              <>
                <span>{isSetupMode ? 'Initialize Administrator Account' : 'Sign In to Workspace'}</span>
