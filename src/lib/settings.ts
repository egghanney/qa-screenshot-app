/**
 * AetherQA Client-Side Settings & Credentials Manager
 * Unifies API key storage across all components and maintains backward compatibility.
 */

const STORAGE_KEY_GEMINI = 'AETHER_GEMINI_API_KEY';
const STORAGE_KEY_SETTINGS = 'aether_qa_settings';
...
export function getStoredGeminiApiKey(): string {
  ...
  try {
    // 1. Check direct key
    const directKey = localStorage.getItem(STORAGE_KEY_GEMINI);
